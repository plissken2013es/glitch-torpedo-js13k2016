...
var shipSpeed = 28, chargeSpeed = 14, numCharges = 6, subScores = [20, 30, 50, 60, 70, 80], subSpeed = 6, torpedoSpeed = 14, numTorpedoes = 6, demoMode = false, playerShipName = "PLAYER 1", playerSubName = "PLAYER 2";

var ship, lastTime, gameTime, isGameOver, charges, chargeExplosions, lastFireCharge, subs, points, subsAvailable,  activeSubs, levels, indicators, indCnt, torpedoes, torpedoExplosions, lastFireTorpedo, scorePlayerShip, scorePlayerSub, timeLeft, timeInterval, bodyCount, role, timeout, mySocket, netCnt;
...
function launchCharge(pos) {
    if (role == "ship") this.mySocket.emit("launchCharge", pos);
...
function launchTorpedo(pos, updateSub) {
    if (role == "sub") {
        this.mySocket.emit("launchTorpedo", pos, updateSub);
    } else if (updateSub) {
...
function launchSub(cfg) {
    if (cfg) console.log("received launchSub from server", cfg.levels);
    if (subsAvailable.length) {
        ...
        console.log("created sub at pos y:", sub.pos[1]);
        sub.dir = dir;
        ...
        if (role == "sub" && !cfg) {
            this.mySocket.emit("launchSub", config);
...
function sendShipPosition() {
    if (netCnt++ == 1) {
        netCnt = 0;
        this.mySocket.emit("shipPosition", ship.pos[0]);
    }
}
...
function updateEntities(dt) {
    ...
    for (i=0; i<subs.length; i++) {
        ...
        if (killSub) {
            ...
            if (role == "sub") {
                points.splice(i, 1);
                subsAvailable.push(indicators.splice(i, 1)[0].value);
            }
            levels.push(s.pos[1]);
        }
    }
